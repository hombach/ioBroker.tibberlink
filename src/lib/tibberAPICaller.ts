...
export class TibberAPICaller extends ProjectUtils {
   ...
   */
  async updateCurrentPriceAllHomes(
  ...
  private async dailyPriceRollover(homeId: string): Promise<void> {
    try {
      ...
      const newPricesToday =
        currentPricesTomorrow.length > 0 ? currentPricesTomorrow : []; // fallback empty
      await this.checkAndSetValue(
        `Homes.${homeId}.PricesToday.json`,
   ...
   * @returns okprice - got correct data
   */
  private async updatePricesToday(
    homeId: string,
    resolution: PriceResolution,
    forceUpdate = false,
  ): Promise<boolean> {
   ...
   * @returns okprice - got new data
   */
  private async updatePricesTomorrow(
    homeId: string,
    resolution: PriceResolution,
    forceUpdate = false,
  ): Promise<boolean> {
    ...
    try {
      for (const home of this.adapter.config.HomesList) {
        ...
        const resolutions = [
          ...
          {
            type: EnergyResolution.HOURLY,
            state: `jsonHourly`,
            numCons: home.numberConsHourly,
            description: `hour`,
          },
          {
            type: EnergyResolution.DAILY,
            state: `jsonDaily`,
            numCons: home.numberConsDaily,
            description: `day`,
          },
          {
            type: EnergyResolution.WEEKLY,
            state: `jsonWeekly`,
            numCons: home.numberConsWeekly,
            description: `week`,
          },
          {
            type: EnergyResolution.MONTHLY,
            state: `jsonMonthly`,
            numCons: home.numberConsMonthly,
            description: `month`,
          },
          {
            type: EnergyResolution.ANNUAL,
            state: `jsonAnnual`,
        ...
        for (const { type, state, numCons, description } of resolutions) {
          if (numCons && numCons > 0) {
            ...
            const consumption: IConsumption[] =
              await this.tibberQuery.getConsumption(type, numCons, homeID);
  ...
  // WiP 872 - add some useful aggregated values for consumption, e.g. total consumption of current month for monthly resolution
  private getCurrentMonthConsumption(
    ...
    const sortedConsumption = consumption
      ...
      .filter(
        (item): item is { entry: IConsumption; date: string } =>
   ...
   */
  private async fetchPriceRemainingAverage(
    homeId: string,
    objectDestination: string,
    price: IPrice[],
  ): Promise<void> {
