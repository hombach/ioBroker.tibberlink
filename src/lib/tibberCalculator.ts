		...
		try {
			...
			switch (this.adapter.config.CalculatorList[channel].chType) {
				case enCalcType.BestCost:
					break;
				case (enCalcType.BestSingleHours, enCalcType.BestHoursBlock):
					break;
				default:
					this.adapter.log.error(`Calculator Type for channel ${channel} nit set, please do!`);
			}
			...
			this.checkAndSetValueBoolean(
				this.getStatePrefix(homeId, `Calculations.${channel}`, `Active`, this.adapter.config.CalculatorList[channel].chName),
